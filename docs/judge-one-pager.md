...
SignalDesk is for the Slack security thread that starts messy: a suspicious link, a possible leaked token, a half-remembered alert, and nobody quite sure who owns the next step. It is built for nonprofits, schools, clinics, and community teams that coordinate in Slack but do not have a full SOC.

The app turns that first report into a Slack incident brief with evidence IDs, claim checks, detection opportunities, first-response roles, guardrails, and an incident channel handoff.
...
2. The brief shows `Runtime: MCP stdio`, so the Slack path is visibly using the MCP `triage_slack_alert` tool.
...
| Quality of the Idea | Focused incident response inside Slack, not a generic chatbot. It preserves assumptions and turns conversation into coordinated action. |
...
- Public GitHub repo URL in the Devpost fields.
